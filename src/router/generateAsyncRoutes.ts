...
LayoutMap.set('RouteView', RouteView)
let dynamicViewsModules: Record<string, () => Promise<Recordable>>
// 设置为单例模式避免多次生成影响效率
// 前端路由表
let constantRouterComponents: { [x: string]: Function } = {
  // 基础页面 layout 必须引入
  BasicLayout,
  BlankLayout,
  RouteView,
...
const generateAsyncRoutes = (menu?: Array<unknown>) => {
  // TODO:使用后端路由时,generateAsyncRoutes方法会被调用10余次,待优化
  let menuNav: Array<any> = [];
  let childrenNav: Array<unknown> = [];
  const routes = getRoutePages()
  constantRouterComponents = { ...constantRouterComponents, ...routes }
  // 后端数据, 根级树数组,  根级 PID
...
export const asyncImportRoute = (view: string) => {
...
export const menuToRouter = (routerMap, parent?) => {
  return routerMap.map(item => {
    ...
    const currentRouter: any = {
      ...
      // component: asyncImportRoute(item.component),
